...
from python_parser.src.parse_content import (
    document, line, header_line, bullet_line, code_block, tag, link
)
...
@pytest.mark.parametrize("input_text,expected", [
    ...
    (
        "Just some text",
        [{'type': 'line', 'content': [{'type': 'text', 'content': 'Just some text'}]}]
    ),
...
@pytest.mark.parametrize("input_text,expected", [
    ("[[http://example.com]]", {'type': 'link', 'address': 'http://example.com', 'text': None}),
    ("[[http://example.com|Example]]", {'type': 'link', 'address': 'http://example.com', 'text': 'Example'}),
])
        ...
        'children': [
            {
                ...
                'title': 'Title',
                'children': [
                    {'type': 'line', 'content': [{'type': 'text', 'content': 'Some text here'}]},
                    ...
                    {'type': 'bullet', 'indent': 0, 'content': 'item 2'},
                    {
                        ...
                        'title': 'Subsection',
                        'children': [
                            {'type': 'line', 'content': [{'type': 'text', 'content': 'Another line'}]},
                        ]
                    }
                ]
            }
